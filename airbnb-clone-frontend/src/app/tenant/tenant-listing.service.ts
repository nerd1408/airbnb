...
export class TenantListingService {
  ...
  getAllByCategory(pageRequest: Pagination, category: CategoryName) : void {
    ...
    this.http.get<Page<CardListing>>(`${environment.API_URL}/tenant-listing/get-all-by-category`)
      .subscribe({
        next: (displayListingCards: any) =>
        {
          return this.getAllByCategory$.set(State.Builder<Page<CardListing>>().forSuccess(displayListingCards));
          },
        error: (error: any) => {
          console.log(error);
          return this.getAllByCategory$.set(State.Builder<Page<CardListing>>().forError(error));
        }
      });
